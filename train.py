...
from utils import read_json, GetImages, img_c2w, get_focal_from_fov
...
f_length = get_focal_from_fov(field_of_view=json_train_data["camera_angle_x"],
                              width=config.IMG_WIDTH)

# Train, val, and test images with their c2w matrices
train_img_path, train_c2w = img_c2w(json_data=json_train_data,
                                    path=config.DATASET)
val_img_path, val_c2w = img_c2w(json_data=json_val_data,
                                path=config.DATASET)
test_img_path, test_c2w = img_c2w(json_data=json_test_data,
                                  path=config.DATASET)

# Get images
images = GetImages(width=config.IMG_WIDTH, height=config.IMG_HEIGHT)

train_data = tf.data.Dataset.from_tensor_slices(train_img_path).map(images, num_parallel_calls=tf.data.experimental.AUTOTUNE)
val_data = tf.data.Dataset.from_tensor_slices(val_img_path).map(images, num_parallel_calls=tf.data.experimental.AUTOTUNE)
test_data = tf.data.Dataset.from_tensor_slices(test_img_path).map(images, num_parallel_calls=tf.data.experimental.AUTOTUNE)

# Instantiate the rays object
rays = Rays(f_length=f_length, width=config.IMG_WIDTH, height=config.IMG_HEIGHT, near_bound=config.NEAR, far_bound=config.FAR,
            nC=config.N_C)

# Define train, val, and test rays datasets
train_ray_data = tf.data.Dataset.from_tensor_slices(train_c2w).map(rays, num_parallel_calls=tf.data.experimental.AUTOTUNE)
val_ray_data = tf.data.Dataset.from_tensor_slices(val_c2w).map(rays, num_parallel_calls=tf.data.experimental.AUTOTUNE)
test_ray_data = tf.data.Dataset.from_tensor_slices(test_c2w).map(rays, num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
# Input pipeline
train_data = train_data.shuffle(config.BATCH_SIZE).batch(config.BATCH_SIZE).repeat().prefetch(tf.data.experimental.AUTOTUNE)
val_data = val_data.shuffle(config.BATCH_SIZE).batch(config.BATCH_SIZE).repeat().prefetch(tf.data.experimental.AUTOTUNE)
test_data = test_data.batch(config.BATCH_SIZE).prefetch(tf.data.experimental.AUTOTUNE)
...
# Compile the model with Adam optimizer and MSE loss according to original paper
...
#print([i for i in train_data.take(1)][0])
